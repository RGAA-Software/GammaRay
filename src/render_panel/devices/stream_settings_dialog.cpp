...
namespace tc
{
    ...
    void StreamSettingsDialog::CreateLayout() {
        ...
        // Force Direct
        {
            ...
            auto label = new TcLabel(this);
            label->setFixedWidth(item_width);
            label->SetTextId("id_force_direct");
            label->setStyleSheet(R"(color: #333333; font-weight: 700; font-size:13px;)");
            label->setAlignment(Qt::AlignLeft|Qt::AlignVCenter);
            layout->addWidget(label);
            layout->addSpacing(10);
            ...
            auto cb = new QCheckBox(this);
            cb->setChecked(stream_item_->force_direct_);
            cb_force_direct_ = cb;
            connect(cb, &QCheckBox::checkStateChanged, this, [=, this](Qt::CheckState state) {
                if (state == Qt::Checked) {
        ...
        // Force software
        {
            ...
            auto label = new TcLabel(this);
            label->setFixedWidth(item_width);
            label->SetTextId("id_force_software");
            label->setStyleSheet(R"(color: #333333; font-weight: 700; font-size:13px;)");
            label->setAlignment(Qt::AlignLeft|Qt::AlignVCenter);
            layout->addWidget(label);
            layout->addSpacing(10);
            ...
            auto cb = new QCheckBox(this);
            cb->setChecked(stream_item_->force_software_);
            cb_force_software_ = cb;
            layout->addWidget(cb);
            ...
            auto tooltip = new TcToolTip(this);
            tooltip->setFixedSize(275, 70);
            tooltip->SetText("Force using Software to render and decode.");
            tooltip->hide();
            btn_tips->SetOnImageButtonHovering([=](QWidget* w) {
                auto w_pos = w->mapToGlobal(QPoint(0,0));
                tooltip->move(w_pos.x() - tooltip->width() - 5, w_pos.y());
        ...
        // force gdi capture
        {
            ...
            auto label = new TcLabel(this);
            label->setFixedWidth(item_width);
            label->SetTextId("id_force_gdi_capture");
            label->setStyleSheet(R"(color: #333333; font-weight: 700; font-size:13px;)");
            label->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
            layout->addWidget(label);
            layout->addSpacing(10);
        ...
        // disable vulkan render 
        {
            ...
            auto label = new TcLabel(this);
            label->setFixedWidth(item_width);
            label->SetTextId("id_disable_vulkan_render");
            label->setStyleSheet(R"(color: #333333; font-weight: 700; font-size:13px;)");
            label->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
            layout->addWidget(label);
            layout->addSpacing(10);
            ...
            auto cb = new QCheckBox(this);
            cb->setChecked(stream_item_->disable_vulkan_render_);
            cb_disable_vulkan_render_ = cb;
            layout->addWidget(cb);
