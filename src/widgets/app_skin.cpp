//
// Created by RGAA on 2024/4/9.
//
#include "app_skin.h"

namespace tc
{

    QColor AppSkin::kTabBtnInActiveColor = QColor(0xeeeeee);

    QColor AppSkin::kNormalColor = QColor(0x7350ce);
    QColor AppSkin::kTabBtnNormalColor = QColor(0x486487);
    QColor AppSkin::kTabBtnHoverColor = QColor(0x2979ff);
    QColor AppSkin::kTabBtnPressedColor = QColor(0x286487);

    QColor AppSkin::kWebWpColor = QColor(0xed5a65);
    QColor AppSkin::kWebWpHoverColor = QColor(0xe3545e);
    QColor AppSkin::kWebWpPressedColor = QColor(0xd94852);

    QColor AppSkin::kVideoWpColor = QColor(0x2486b9);
    QColor AppSkin::kVideoWpHoverColor = QColor(0x2581b1);
    QColor AppSkin::kVideoWpPressedColor = QColor(0x1c79a9);

}