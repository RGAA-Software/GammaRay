...
namespace tc {
    ...
    void OverlayWidget::paintEvent(QPaintEvent*) {
        ...
        // 1. 根据窗口大小自动调整字体
        // -------------------------
        const int baseFontPx = 20;     // 参考字体大小
        const int refHeight = 800;     // 参考高度，可改
        ...
        for (int r = 0; r < gridRows && idx < watermark_count_; r++) {
            for (int c = 0; c < gridCols && idx < watermark_count_; c++) {
                ...
                int baseX = c * cellW + cellW / 2;
                int baseY = r * cellH + cellH / 2;

                int x = baseX + randOffset(cellW / 3);
                int y = baseY + randOffset(cellH / 3);

                QRect textRect(x - 400, y - fontPx * 2, 800, fontPx * 4);
