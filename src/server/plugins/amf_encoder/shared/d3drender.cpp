...
namespace
{
	...
	bool CreateDevice( IDXGIAdapter *pDXGIAdapter, ID3D11Device **pD3D11Device, ID3D11DeviceContext **pD3D11Context )
	{
		...
		if ( FAILED( hRes ) )
		{
			LOGE( "Failed to create D3D11 device! (err=%u)", hRes );
			return false;
		}

		if ( eFeatureLevel < D3D_FEATURE_LEVEL_11_0 )
		{
			LOGE( "DX11 level hardware required!" );
			return false;
		}
		...
		if (SUCCEEDED(hr)) {
			LOGI("Successfully get ID3D11Multithread interface. We set SetMultithreadProtected(TRUE)");
			D3D11Multithread->SetMultithreadProtected(TRUE);
			D3D11Multithread->Release();
		}
		else {
			LOGE("Failed to get ID3D11Multithread interface. Ignore.");
		}
