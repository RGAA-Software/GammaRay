...
{
	m_hMapFile = OpenFileMappingA(FILE_MAP_READ, false, pName);
}

IPCFileMapping::IPCFileMapping(const char* pName, uint64_t size)
{
	m_hMapFile = CreateFileMappingA(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, (DWORD)(size >> 32), (DWORD)size, pName);
}
