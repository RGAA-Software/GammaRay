#include "sig_sdk_router_ws_impl.h"
...
namespace tc
{
    ...
    SigRouterWsImpl::SigRouterWsImpl(const std::shared_ptr<RtcContext>& ctx) : SigRouterInterface(ctx) {

    }

    void SigRouterWsImpl::Init(const SignalingParam& params) {
        sig_params_ = params;
    }

    void SigRouterWsImpl::Start() {
    ...
    void SigRouterWsImpl::SendSigMessage(const std::string& sig_name, const std::string& token, const std::string& msg) {
        if (!IsAlive()) {
            return;
        }
    ...
    void SigRouterWsImpl::SendHeartBeat() {
        if (!IsAlive()) {
            return;
        }
